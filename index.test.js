...
describe("Render task definition", () => {
            ...
            secrets: [
              {
                ...
                valueFrom:
                  "arn:aws:ssm:region:0123456789:parameter/existingSecret",
              },
              {
                name: "SSM_SECRET",
                valueFrom:
                  "arn:aws:ssm:region:0123456789:parameter/oldSsmSecret",
              },
            ],
          },
          {
          ...
          containerDefinitions: [
            {
              ...
              secrets: [
...

                {
                  name: "SM_SECRET",
                  valueFrom:
                    "arn:aws:secretsmanager:us-east-1:0123456789:secret:secretName",
                },
              ],
            },
            {
  ...
  test("error returned for malformatted secret string", async () => {
    ...
    expect(core.setFailed).toBeCalledWith(
      expect.stringContaining(`Cannot parse the secret 'SECRET'`),
    );
  });
  ...
  test("renders secrets without environment-variables", async () => {
    ...
    jest.mock(
      "/hello/secrets-only-task-definition.json",
      () => ({
        family: "task-def-family",
        containerDefinitions: [
          {
          ...
          containerDefinitions: [
            {
              ...
              secrets: [
                ...
                },
                {
                  name: "DB_PASSWORD",
                  valueFrom:
                    "arn:aws:ssm:us-east-1:123456789:parameter/db-password",
                },
              ],
            },
          ],
  ...
  test("overrides top-level task definition fields from a file input", async () => {
    ...
    const renderedTaskDefinition = JSON.parse(
