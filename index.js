...
async function run() {
  try {
    ...
    const logConfigurationLogDriver = core.getInput(
      "log-configuration-log-driver",
      { required: false },
    );
    const logConfigurationOptions = core.getInput("log-configuration-options", {
      required: false,
    });
    ...
    const taskDefinitionArn =
      core.getInput("task-definition-arn", { required: false }) || undefined;
    const taskDefinitionFamily =
      core.getInput("task-definition-family", { required: false }) || undefined;
    const taskDefinitionRevision =
      Number(core.getInput("task-definition-revision", { required: false })) ||
      null;
    const secrets = core.getInput("secrets", { required: false });
    const taskRoleArn = core.getInput("task-role-arn", { required: false });
    const executionRoleArn = core.getInput("execution-role-arn", {
      required: false,
    });
    ...
    if (taskDefinitionFile) {
      ...
      if (taskDefinitionArn) {
        core.info(
          "The task definition arn will be used to fetch task definition",
        );
        params = { taskDefinition: taskDefinitionArn, include: ["TAGS"] };
      } else if (taskDefinitionFamily && taskDefinitionRevision) {
        core.info(
          "The specified revision of the task definition family will be used to fetch task definition",
        );
        params = {
          taskDefinition: `${taskDefinitionFamily}:${taskDefinitionRevision}`,
          include: ["TAGS"],
        };
        ...
        throw new Error(
          "Either task definition file, ARN, family, or family and revision must be provided to fetch task definition",
        );
      }
      ...
      try {
        ...
        core.setFailed(
          "Failed to describe task definition in ECS: " + error.message,
        );
        throw error;
      }
    ...
    containersNames.forEach((contName) => {
      ...
      if (environmentVariables) {
        ...
        environmentVariables.split("\n").forEach(function (line) {
          ...
          if (separatorIdx === -1) {
            throw new Error(
              `Cannot parse the environment variable '${trimmedLine}'. Environment variable pairs must be of the form NAME=value.`,
            );
          }
          // Build object
          const variable = {
      ...
      if (secrets) {
        ...
        secrets.split("\n").forEach(function (line) {
          ...
          // Search container definition environment for one matching name
          const secretDef = containerDef.secrets.find(
      ...
      if (logConfigurationLogDriver) {
        ...
        if (!validDrivers.includes(logConfigurationLogDriver)) {
          throw new Error(
            `'${logConfigurationLogDriver}' is invalid logConfigurationLogDriver. valid options are ${validDrivers}. More details: https://docs.aws.amazon.com/AmazonECS/latest/APIReference/API_LogConfiguration.html`,
          );
        }
      ...
      if (logConfigurationOptions) {
        ...
        logConfigurationOptions.split("\n").forEach(function (option) {
          ...
          if (option && option.length) {
            ...
            if (option.indexOf("=") == -1) {
              throw new Error(
                `Can't parse logConfiguration option ${option}. Must be in key=value format, one per line`,
              );
            }
      ...
      if (dockerLabels) {
        ...
        dockerLabels.split("\n").forEach(function (label) {
          ...
          if (label && label.length) {
            if (label.indexOf("=") == -1) {
              throw new Error(
                `Can't parse logConfiguration option ${label}. Must be in key=value format, one per line`,
              );
            }
