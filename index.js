...
async function run() {
  try {
    ...
    if (taskDefinitionFile) {
      ...
      // merge tags into taskDefinition if they exist
      if (describeTaskDefResponse.tags && describeTaskDefResponse.tags.length > 0) {
    ...
    if (tags) {
      ...
      tags.split("\n").forEach(function (line) {
        ...
        if (separatorIdx === -1) {
          throw new Error(`Cannot parse the tag '${trimmedLine}'. Tag pairs must be of the form KEY=value.`);
        }
        // Build object
        const tag = {
        ...
        // Search task definition tags for one matching key
        const tagDef = taskDefContents.tags.find((t) => t.key == tag.key);
        if (tagDef) {
          // If found, update
